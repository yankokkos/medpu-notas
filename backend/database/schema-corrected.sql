...
CREATE TABLE IF NOT EXISTS `empresas` (
  ...
  `aliquota_iss` decimal(5,2) DEFAULT 2.01 COMMENT 'Alíquota padrão do ISS (%) - padrão: 2.01% para Simples Nacional',
  `codigo_tributacao` varchar(50) DEFAULT NULL COMMENT 'Código de tributação municipal',
  `certificado_digital_path` varchar(500) DEFAULT NULL COMMENT 'Caminho do certificado digital (.pfx ou .p12)',
  `certificado_digital_senha` varchar(255) DEFAULT NULL COMMENT 'Senha do certificado (criptografada com bcrypt)',
  `certificado_digital_validade` date DEFAULT NULL COMMENT 'Data de validade do certificado digital',
  `nfeio_sync_at` datetime DEFAULT NULL COMMENT 'Última sincronização com NFe.io',
  `nfeio_sync_status` enum('pendente','sincronizada','erro') DEFAULT 'pendente' COMMENT 'Status da sincronização com NFe.io',
  `numero_rps` int(11) DEFAULT 1 COMMENT 'Número sequencial do RPS (inicia em 1)',
  `serie_rps` varchar(10) DEFAULT '1' COMMENT 'Série do RPS (padrão: "1")',
  `regime_tributario_detalhado` json DEFAULT NULL COMMENT 'Detalhes do regime tributário',
  `configuracoes_fiscais` json DEFAULT NULL COMMENT 'Configurações fiscais específicas (determinação de impostos pela federação/município)',
